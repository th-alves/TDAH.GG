...
body::after {
  ...
  position: fixed; inset: 0;
  background:
    radial-gradient(ellipse at 15% 40%, rgba(108,99,255,0.12) 0%, transparent 55%),
    radial-gradient(ellipse at 85% 20%, rgba(0,212,255,0.08) 0%, transparent 50%),
    radial-gradient(ellipse at 50% 90%, rgba(255,45,120,0.06) 0%, transparent 50%);
  pointer-events: none;
  z-index: 0;
}
...
.hero__title {
  ...
  /* Container dos dois slots animados */
  position: relative;
  min-height: clamp(140px, 28vw, 300px);
  display: flex;
  align-items: center;
...
/* Slot A: "TDAH.GG" — aparece primeiro, some depois */
.hero__title-slot--a {
  animation: titleSlotA 9s ease-in-out 2.5s infinite both;
}

/* Slot B: frase — surge depois, fica, some */
.hero__title-slot--b {
  opacity: 0; /* estado inicial antes da animação */
  animation: titleSlotB 9s ease-in-out 2.5s infinite both;
}

@keyframes titleSlotA {
  0%,    38%  { opacity: 1; transform: translateY(0); }
  50%         { opacity: 0; transform: translateY(-18px); }
  88%,  100%  { opacity: 0; transform: translateY(-18px); }
  /* sem volta — slot A some e slot B fica até reiniciar */
}

/* Reescreve para ficar mais legível: A visível → A some → B aparece → B some → A */
@keyframes titleSlotA {
  0%          { opacity: 1; transform: translateY(0); }        /* início: visível */
  38%         { opacity: 1; transform: translateY(0); }        /* segura */
  46%         { opacity: 0; transform: translateY(-18px); }    /* some */
  90%         { opacity: 0; transform: translateY(-18px); }    /* fica oculto */
  100%        { opacity: 1; transform: translateY(0); }        /* volta (loop) */
}

@keyframes titleSlotB {
  0%          { opacity: 0; transform: translateY(18px); }     /* início: oculto */
  38%         { opacity: 0; transform: translateY(18px); }     /* segura */
  50%         { opacity: 1; transform: translateY(0); }        /* aparece */
  88%         { opacity: 1; transform: translateY(0); }        /* segura */
  98%         { opacity: 0; transform: translateY(18px); }     /* some */
  100%        { opacity: 0; transform: translateY(18px); }     /* fim */
}
